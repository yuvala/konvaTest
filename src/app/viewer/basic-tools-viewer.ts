//import { PointF } from "./interfaces/ImageDataModels";
...
interface IViewer {
    ...
    Init($window, logger: ILog, $rootScope): void;

    SetImageUrl(imageUrl: string, originalWidth, originalHeight, baseWLWindow, baseWLLevel, imageLoadedCallback, imageErrorLoadedCallback): void;
    SetWLImageDataUrl(wlImageDataUrl: string, wlImageDataLoadedCallback, OnWindowLevelsChangeCallback: (wlWindow: number, wlLevel: number) => void): void;
    SetOneTouchPan(enabled: boolean): void;
    WindowLevel(onNewImageLoadedWithWLStarted, onNewImageLoadedWithWLFinished, photometricInterpretation): void;
    GetWLWindow(): number;
    GetWLLevel(): number;
    FitImage(): void;
    setElementSize(): void;
    ...
    ClearPlanning();
    Cleanup(): void;
    SetCurrentSelected(shape): void;
    SetGroupSelected(shape: string): void;

    EventShapeRemoved: string;

    moveObjectToTop(shape): void;
    traverseStageOnClick(container, excludeContainer): void;
